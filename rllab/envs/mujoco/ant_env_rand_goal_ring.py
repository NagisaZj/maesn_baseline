...
class AntEnvRandGoalRing(MujocoEnv, Serializable):
    ...
    def __init__(self, goal= None, sparse= False, train = True,  *args, **kwargs):
        ...
        if train :
            assert sparse == False
            self.goals = pickle.load(open('/home/zj/Desktop/maesn_suite/maesn/rllab/envs/goals/ant_trainSet.pkl', "rb"))
        else:
            assert sparse == True
            self.goals = pickle.load(open('/home/zj/Desktop/maesn_suite/maesn/rllab/envs/goals/ant_valSet.pkl' , 'rb'))
        self.sparse = sparse
        super(AntEnvRandGoalRing, self).__init__(*args, **kwargs)
        Serializable.__init__(self, *args, **kwargs)
        print(type(self.observation_space),self.observation_space,self.observation_space.flat_dim)
